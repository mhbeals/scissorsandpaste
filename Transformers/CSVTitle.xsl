...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="teiCorpus"><xsl:for-each-group select="TEI" group-by="teiHeader/fileDesc/sourceDesc/biblFull/titleStmt/title">
        <xsl:result-document method="text" href="Outputs\CSV\Title\{teiHeader/fileDesc/sourceDesc/biblFull/titleStmt/title}.csv">
            Article ID,Newspaper Title,Newspaper City,Newspaper Province,Newspaper Country,Year,Month,Day,Article Type,Text,Keywords<xsl:for-each select="current-group()">
