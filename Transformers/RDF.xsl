...
sapcorpus:<xsl:value-of select="."/> a fabio:Newspaper ;
</xsl:for-each>
...
sapcorpus:article_<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/@n"/>
	...
	fabio:partOf sapcorpus:<xsl:value-of select="teiHeader/fileDesc/sourceDesc/biblFull/titleStmt/title/@n"/>_<xsl:value-of select="teiHeader/fileDesc/sourceDesc/biblFull/publicationStmt/date/@when"/> ;
	<xsl:if test="text/body/p[1]/pb/@n!=''">prism:startingPage "<xsl:value-of select="text/body/p[1]/pb/@n"/>" ;</xsl:if><xsl:if test="text/body/bibl/@type='original'">
	...
	frbr:embodiment &lt;<xsl:value-of select="teiHeader/fileDesc/sourceDesc/biblFull/publicationStmt/authority/ref/@target"/>&gt; .
</xsl:if>
...
&lt;<xsl:value-of select="teiHeader/fileDesc/sourceDesc/biblFull/publicationStmt/authority/ref/@target"/>&gt; frbr:embodimentOf sap:article_<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/@n"/>.
</xsl:if>
...
sapcorpus:image_<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/@n"/>
	...
	pav:retrievedOn "<xsl:value-of select="teiHeader/fileDesc/publicationStmt/authority/date[1]/@when"/>"^^xsd:date. ].
...
<!-- XML of Article -->&lt;http://www.scissorsandpaste.net/<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/@n"/>.xml&gt;
	...
	sap:transcribedOn "<xsl:value-of select="teiHeader/fileDesc/publicationStmt/authority/date[1]/@when"/>"^^xsd:date ;			<xsl:if test="teiHeader/fileDesc/publicationStmt/authority/persName[3] !=''">pav:catagorisedBy sap:<xsl:value-of select="teiHeader/fileDesc/publicationStmt/authority/persName[3]/@n"/> ;
	...
	fabio:isRepresentationOf sap:article_<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/@n"/> ;
	fabio:isStoredOn fabio:internet ;<xsl:for-each select="teiHeader/profileDesc/textClass/keywords/list/item">
	<xsl:if test="contains(@n,'sh')">dc:coverage &lt;http://id.loc.gov/authorities/subjects/<xsl:value-of select="@n"/>.html&gt;</xsl:if></xsl:for-each><xsl:if test="text/body/head/settlement != ''"><xsl:for-each select="text/body/head/settlement"> ;
	...
	sap:body "<xsl:for-each select="text/body/p"><xsl:value-of select="."/><xsl:text>&#160;</xsl:text></xsl:for-each>" .
	</xsl:for-each>
