...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="teiCorpus">
  <xsl:result-document method="text" href="CSV Entries\TEICND.csv">Article ID,Newspaper Title,Newspaper City,Newspaper Province,Newspaper Country,Year,Month,Day,Article Type,Text,Keywords<xsl:for-each select="TEI">
        ...
        select="teiHeader/fileDesc/sourceDesc/biblFull/publicationStmt/date"><xsl:value-of 
         select="substring(@when,1,4)"/>,<xsl:value-of select="substring(@when,6,2)"/>,<xsl:value-of 
          select="substring(@when,9,2)"/>,</xsl:for-each><xsl:value-of select="teiHeader/profileDesc/textDesc/purpose/@type"/>,<xsl:for-each 
           select="text/body/p"><xsl:value-of select="normalize-space(translate(.,'&quot;,Â',''))"/><xsl:text>&#160;</xsl:text></xsl:for-each>,<xsl:for-each
